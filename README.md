...
发布前会校验本地素材。对 MP4/M4V/MOV，Skill 不依赖 ffmpeg 即可读取时长；抖音超过实测 15:00 边界时会在打开浏览器前明确拦截，不会自动裁剪或转码，也不会影响同一素材继续准备其他平台。

## 自定义发布流程

如果需要增加自己的步骤，例如在某个平台填写完标签后再点击一个设置按钮，可以直接让 Agent 按[自定义发布流程扩展规范](video-publisher/references/customizing-workflows.md)处理。用户只需要说明平台、触发时机、按钮文字或附近区域，以及什么页面状态代表成功；规范会指导 Agent 将它实现为可恢复、可重复运行并经过真实页面验证的流程。
