<p align="center">
  <img src="./assets/readme/hero.svg" width="100%" alt="Video Publisher：用 Ego Lite 准备视频草稿，默认停在最终发布前">
</p>

<p align="center">
  小红书 · 抖音 · Bilibili · 微信视频号 · YouTube<br>
  <strong>并行上传，完成即验收，明确授权后才发布</strong>
</p>

将视频、标题和封面准备为经过核验的多平台草稿，支持恢复上传，默认停在最终发布之前。

> [!IMPORTANT]
> Skill 默认不会点击最终发布按钮。只有用户明确要求把本次内容发布到指定平台，才会重新核对草稿并完成最终发布；准备草稿、上传和长期账号配置不算授权。
...
- 草稿准备阶段会挂载最终发布按钮保护；保护未成功启用时不能返回 `READY`。只有用户明确要求发布本次内容，Agent 才能在重新核对草稿后解除当前页面的保护。
...
- 最终发布分支目前通过本地守卫测试，尚未进行真实平台的最终点击验收；首次使用应逐个平台核对发布回执。

## 已知边界

- 这是基于真实网页的浏览器自动化，依赖平台登录状态，也可能受到平台改版、风控或服务波动影响。
- Skill 不负责剪辑、转码或制作封面；最终发布是单独的用户授权分支，不属于默认草稿流程。
