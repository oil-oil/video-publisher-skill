<p align="center">
  <img src="./assets/readme/hero.svg" width="100%" alt="Video Publisher：用 Ego Lite 将一个本地视频准备为四个平台的可发布草稿，并始终停在最终发布前">
</p>
...
| ---: | ---: | ---: | ---: |
| 4 个平台 | 19 轮 | 68 / 68 | 0 次 |

真实回归覆盖了四平台冷启动、连续无操作重跑、旧草稿隔离、上传中断、填写中断、Ego Lite 崩溃、任务空间删除与 ID 复用、状态文件损坏、重复任务竞争、超大视频和抖音 15 分钟边界等场景。

这些数字代表页面适配和安全合同已经积累的真实测试，不是对平台永不改版的承诺。2026-07-17 新增的 rolling 调度器已经通过本地集成测试，但仍等待下一轮完整真实四平台回归。页面发生变化时，Skill 会返回可诊断的阻塞状态，不会把“点过了”误报成 `READY`。
...
| Bilibili | 标题、简介、标签、自制声明、可选 16:10 封面、旧草稿隔离 |
...
3. **完成即处理**：任一平台完成上传后，立即进入唯一的串行 UI 队列，填写标题、标签、声明和封面，再独立验收。
