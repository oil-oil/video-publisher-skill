...
description: "将本地视频、标题和封面准备为小红书、抖音、B站、视频号或 YouTube 的已验证草稿，支持账号配置、恢复上传和草稿检查。用户要求准备或管理发布草稿时使用，默认停在最终发布按钮前；用户明确要求发布已核对的本次内容到指定平台时，也使用本 Skill 完成最终发布。不用于视频剪辑、封面制作、公众号长文或一般平台建议。"
---

# 视频发布

确认一个视频内容包，交给维护好的程序完成多平台上传、表单修复和独立验证。默认把草稿留在最终发布按钮前；用户明确要求发布当前内容时，才进入下文的最终发布分支。

## 必须遵守

- 将本文件所在目录记为 `SKILL_DIR`，所有相对命令都在该目录运行。
- 日常发布只调用维护好的生产入口，不手工拆分平台阶段，也不自行控制创作者页面。
- 准备草稿和验收期间不点击最终发布、保存或定时发布按钮。只有用户在当前任务明确要求将已确认的内容发布到具体平台，才进入最终发布分支；“准备草稿”“上传”“做好发布准备”和长期账号配置都不构成最终发布授权。
...
这是草稿准备完成后的独立分支，不修改生产入口的默认安全行为。只对本次明确指定的内容与平台执行；当前对话已经确认的内容与平台可沿用，缺少或含糊时先问清。原创权利、账号身份、视频、标题、封面、可见性和平台设置仍须与用户已确认的内容一致。

1. 先用同一 Job 的 `--inspect-only --job-id <原 Job ID>` 重新核对目标平台，要求该平台仍为 `READY`、草稿身份一致且最终按钮可见。其他平台没有授权就不处理。若用户接管空间、页面改变、草稿缺失或任何核对失败，停止，不创建新草稿代替。
2. 在对应的 Ego 任务空间中再次核对页面上的账号、视频、标题、封面和发布范围。然后仅在该页面执行 `window.__VIDEO_PUBLISHER_FINAL_GUARD__?.releaseForConfirmedPublish?.()`，必须返回 `ok: true`；它只解除当前页面的最终按钮拦截，不代表已经发布。不要在准备或诊断阶段调用，也不要用刷新页面绕过守卫。解除后只点击与用户请求相符的最终发布控件；YouTube 的 `Save`、`Publish`、`Schedule` 含义不同，不能互换。出现新的确认弹窗时核对其内容与公开范围后再继续。
3. 点击后读取平台给出的成功状态、作品链接或作品列表中的匹配记录。没有可验证的发布证据时报告“发布结果未确认”，不得再次点击或另建草稿来猜测结果。一次授权只用于这次内容与这些平台；不要保存成长期自动发布设置。

交付时区分“本地封面已生成”“平台已接收封面”“草稿 READY”“已点击最终发布”和“平台已确认发布”。历史 READY 不能证明页面仍在；用户报告页面缺失时先检查原任务，不能直接报完成或默认重传。已发布的平台不再恢复上传。
