...
During draft preparation, never click a final `发布`, `发布笔记`, `发表`, `立即投稿`, `保存`, `安排时间`, `Save`, `Publish`, or `Schedule` button. Every adapter result must leave `finalPublishClicked: false`. The shared core installs a capture-phase click/submit guard; draft safety passes only when `guardArmed: true` and `blockedAttempts: 0` are observed from the live page. After a fresh READY inspection, the calling Agent may release this guard on the exact page only for a user-confirmed final publication, following the separate branch in `SKILL.md`; this is never an adapter phase or a standing preference.
