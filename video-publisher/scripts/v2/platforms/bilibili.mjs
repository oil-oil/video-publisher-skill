...
async function uploadBilibili(){let before=await inspectBilibili();if(before.gates.video.ok)return before;if(before.gates.video.evidence?.restoreBanner){await resumeBilibiliLocalDraftIfPresent();await wait(3);before=await inspectBilibili();if(before.gates.video.ok)return before;}if(!before.gates.draftIdentity.ok)return {...before,blocker:typedBlocker('FOREIGN_DRAFT','B站当前编辑器属于其他视频草稿',{retryable:true,evidence:before.gates.draftIdentity.evidence})};const exposed=await js(String.raw`(() => {const input=[...document.querySelectorAll('input[type=file]')].find(el=>/\.(mp4|flv|avi|wmv|mov|webm|mkv|m4v|ts|mpg|rmvb)\b/i.test(el.accept||''));if(!input)return {ok:false,reason:'bilibili video input missing'};input.id='vp2-bili-video';return {ok:true,selector:'#vp2-bili-video'}})()`);if(!exposed.ok)return {...before,blocker:typedBlocker('SELECTOR_DRIFT',exposed.reason)};try{await uploadFile(exposed.selector,videoPath)}catch(error){return {...before,blocker:typedBlocker('UPLOAD_NOT_STARTED',String(error?.message||error),{retryable:true})}}let stableSince=0;for(let i=0;i<180;i+=1){const current=await inspectBilibili();if(current.gates.video.ok){if(!stableSince)stableSince=Date.now();if(Date.now()-stableSince>=10000)return current}else stableSince=0;await wait(5)}const after=await inspectBilibili();return {...after,blocker:typedBlocker('UPLOAD_STALLED','B站视频没有在等待窗口内稳定完成',{retryable:true,evidence:after.gates.video.evidence})}}
